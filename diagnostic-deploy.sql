-- Diagnostic and Robust Database Deployment
-- This script includes checks and handles errors step by step
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.schemata WHERE schema_name = 'auth') THEN
        RAISE EXCEPTION 'Auth schema not found. This must be run in Supabase.';
    END IF;
    RAISE NOTICE 'Auth schema found ✓';
END $$;
...
-- Create profiles table (core dependency)
...
-- Create plans table (no dependencies)
...
RAISE NOTICE 'Social accounts table created ✓';
...
RAISE NOTICE 'Growth reports table created ✓';
...
RAISE NOTICE 'Affiliates table created ✓';
...
RAISE NOTICE 'Affiliate payments table created ✓';
...
RAISE NOTICE 'Coupons table created ✓';
...
-- Trigger for new user registration (create AFTER tables exist)
...
BEGIN
    ...
    AND table_name IN ('profiles', 'social_accounts', 'plans', 'subscriptions', 
                       'growth_reports', 'affiliates', 'referrals', 'affiliate_payments', 
                       'coupons', 'orders', 'order_items');
    
    IF table_count = 11 THEN
        RAISE NOTICE '🎉 SUCCESS: All 11 tables created successfully!';
    ELSE
        RAISE EXCEPTION 'Only % out of 11 tables were created', table_count;
    END IF;
END $$; 